...
## 2024-06-09 - Sanitization of user-supplied URLs

**Vulnerability:** User-provided URLs (e.g., website, linkedin links) were directly rendered in `href` attributes, allowing potential Stored Cross-Site Scripting (XSS) via `javascript:` or `data:` URIs.
**Learning:** The application lacked a central utility for validating and sanitizing user URLs before rendering them in the UI.
**Prevention:** Always use a utility function (like `sanitizeUrl`) to ensure user-supplied URLs have safe protocols (`http:`, `https:`, `mailto:`) before rendering them in `href` attributes.
