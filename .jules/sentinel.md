...
## 2025-05-29 - [Unsanitized `href` User Inputs]
**Vulnerability:** User-provided URLs (`profile.website`, `profile.linkedin`) were directly rendered into `href` attributes in `PublicProfile.tsx` and `ProjectDetails.tsx`.
**Learning:** React prevents XSS when injecting text, but it does NOT sanitize `href` attributes automatically against `javascript:` or `data:` URIs, leading to XSS vulnerabilities.
**Prevention:** Always wrap user-provided URLs in a sanitization utility (like the created `sanitizeUrl` function) that strictly allows safe protocols (`http:`, `https:`, `mailto:`) before rendering them in an anchor tag.
