...
serve(async (req) => {
  ...
  try {
    ...
    // Authenticate client if trying to perform user-attributed action
    if (event === "message") {
      ...
      // Skip JWT validation if token is the service role key (e.g. from webhook/backend)
      if (token !== SUPABASE_SERVICE_ROLE_KEY) {
        ...
        // Ensure they are only sending it on their own behalf
        if (user.id !== user_id) {
