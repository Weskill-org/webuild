...
serve(async (req) => {
  ...
  try {
    ...
    // Impersonation check for sensitive events
    if (!isServiceRole && event === "message") {
      if (!authUser || authUser.id !== user_id) {
        return new Response(JSON.stringify({ error: "Unauthorized. You cannot send messages on behalf of another user." }), {
          status: 403,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
