...
export default function SkillQuizzes() {
  ...
  // ⚡ Bolt: Consolidate multiple O(N) filters over paletteStatuses into a single O(N) reduce pass
  // and memoize the result at the top level to avoid hook rule violations and prevent re-calculation on every tick.
  const paletteStatusCounts = useMemo(() => {
  ...
  if (activeQuiz && examMode === "active") {
    ...
    const counts = paletteStatusCounts;
