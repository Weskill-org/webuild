...
export default function SkillQuizzes() {
  ...
  if (activeQuiz && examMode === "active") {
    ...
    // ⚡ Bolt: Consolidate unmemoized array filtering passes into a single O(N) loop
    // Avoids 5x N iterations on every second the timer ticks during the active exam.
    // Omitted useMemo as hoisting it above early returns violates Rules of Hooks,
    // and the O(N) loop is fast enough on its own.
