...
const EarningsChart = React.memo(function EarningsChart({ transactions }: EarningsChartProps) {
  // ⚡ Bolt: Wrapped chart data parsing and grouping in useMemo
  // 🎯 Why: Recharts SVG rendering and date parsing are computationally expensive. In the Dashboard view, unrelated state changes (like typing in the search bar) were causing this entire pipeline to recalculate and Recharts to re-render, leading to dropped frames.
  // 📊 Impact: O(1) rendering time for the chart during dashboard filter updates, as it only recalcs when transaction array reference changes.
